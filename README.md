...
операторы q и qq, для обозначения соответственно одинарных или двойных кавычек.
...
**Точка с запятой.**
Каждая команда отделяется от других точкой с запятой. Команды могут быть записаны в одну строку. Точка с запятой не обязательна, только если оператор является последним в блоке
...
В качестве разделителей, вместо "/" могут использоваться любые символы. Первый символ устанавливает разделитель.

Флаги не обязательны.

В примере, в операторе "y", SEARCHLIST/REPLACEMENTLIST заданы с помощью регулярных выражений диапазонами:
...
Последние точки над i. Функция print имеет синтаксис
